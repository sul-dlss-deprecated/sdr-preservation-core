...
describe DatastreamRemover do
  ...
  context "basic behavior" do
    ...
    it "can instantiate" do
      dr = DatastreamRemover.new(SEDORA_URI)
    ...
    it "removes a datastream" do
      ...
      pid_namespace = 
      dr = DatastreamRemover.new(SEDORA_URI)
      foo = RestClient.get "#{SEDORA_URI}/objects/#{@fixture_pid}/datastreams/#{datastream_name}?versionable=false"
      foo.code.should eql(200)
      dr.removeDatastream(datastream_name, batch_size, "fixture")
      lambda { RestClient.get "#{SEDORA_URI}/objects/#{@fixture_pid}/datastreams/#{datastream_name}?versionable=false" }.should raise_exception(/404/)      
    end
